...
app.use(
  helmet({
    contentSecurityPolicy: false, // Allowed for inline assets in dev
...
// 4. CORS & Payload Size Limit
app.use(cors());
app.use(express.json({ limit: '10kb' })); // Prevents large payload memory exhaustion
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'online',
    security: 'Top-Class Hardened (Helmet, RateLimit, MongoSanitize, Anti-XSS Active)',
...
app.listen(PORT, () => {
  console.log(`🛡️ Top-Class Hardened Express Server running on http://localhost:${PORT}`);
